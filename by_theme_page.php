...
class TimeMachineManager {
    ...
    function getNewsByTheme($limit = NULL) {
        if (!$limit) $limit = '';
        else $limit = "LIMIT $limit";
        $GET_array = $this->getGET_DateData();
        $statement = $this->pdo->prepare("SELECT news.* FROM news , themes , news_theme WHERE themes.id = {$_GET['themeID']} AND  news_theme.theme_id = themes.id AND news_theme.news_id = news.id");
        $statement->execute([$limit]);
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);
        $this->newsByTheme = $result;
        return $result;
//        var_dump($result);
    }
}
...
$tmm->getNewsByTheme();
//$tmm->getNewsByDate();
$tmm->tagByNewsID = $tmm->getTagByNews($tmm->newsByTheme[0]['id']);
$tmm->imagesByNewsID = $tmm->getIMGBYNewsID($tmm->newsByTheme[0]['id']);
$tmm->themeByNewsID = $tmm->getThemeByNewsID($tmm->newsByTheme[0]['id']);
$tmm->videoByNewsID = $tmm->getVideoByNewsID($tmm->newsByTheme[0]['id']);
...
<head>
    ...
    <title>публикации за дату</title>
    <!--bootstrap-->
    ...
    <div id="time_machine-main-content-container">
        <div id="time_machine-news_card">
            <div class="news_card-caption">
                <div class="chronometr"><?php echo $tmm->newsByTheme[0]['cdt'] ?></div>
                <?php

                $tmm->getTagByNews($tmm->newsByTheme[0]['id']);
            ...
            <div class="time-machine-wrapper-main-body">
                <div class="time-machine-column-wrapper-left">
                    ...
                    <div class="news_card-content-container">
                        ...
                        <div class="time-machine-card-content">
                            <?php echo $tmm->newsByTheme[0]['body'] ?>
                        </div>
                        <div class="time-machine-card-source-container">
                            <?php
                            echo "<a target='_blank' class='time-machine-card-source' href='";
                            echo $tmm->newsByTheme[0]['source_from'] ;
                            echo "'>";
                            if($tmm->newsByTheme[0]['source_from'] !== ''):
                                echo parse_url($tmm->newsByTheme[0]['source_from'])['host'];
                            else:
                ...
                <div class="time-machine-column-wrapper-right">
                    ...
                    <div class="all-hash-tags-by-date">
                        ...
                        $totalAllTagsToDate = [];
                        foreach ($tmm->newsByTheme as $row) {
                    ...
                    <div class="time-machine-amount-container">
                        <span class="time-machine-news-amount"><?= count($tmm->newsByTheme) ?></span>
                        <span class="time-machine-string"> публикаций по теме </span>
                        <span class="time-machine-about-news-theme">
                            <?= $tmm->themeByNewsID[0]['theme_name'] ?>
                        </span>
                    </div>
                    ...
                    <div class="time-machine-last-news-feed">
                        ...
                        foreach ($tmm->newsByTheme as $row) {
                            ...
                            echo "<span class='time-machine-news-feed-item-date'>" . $row['cdt'] . "</span>" ;
                            echo "</div>" ;
        ...
        class GetMoreNewsButton {
            ...
            constructor() {
                ...
                this._buttonSelector.addEventListener('click', () => {
                    ...
                    xhr.open('POST', 'handler_timeMachine_getMoreNewsByTheme.php');
